/**
 * patch-saber-mesh.js
 * Rewrites src/SaberExporter.ts to use MESH attachments (quad geometry)
 * instead of region attachments.  Each bone segment gets a quad mesh with
 * 4 vertices laid out in the bone's local space, UV-mapped to a single
 * glow-bar texture.  RGBA ripple animation remains.
 */
const fs   = require('fs');
const path = require('path');

const OUT = path.join(__dirname, 'src', 'SaberExporter.ts');
...
// ─── single-frame glow-bar texture ───────────────────────────────────────────
// 256 × 128 px: bright core in the horizontal centre, glow falloff top/bottom.
...
function renderBeam(glowHex: string, coreHex: string, falloff: number) {
  ...
  for (let y = 0; y < BEAM_H; y++) {
    ...
    for (let x = 0; x < BEAM_W; x++) {
      // slight horizontal shimmer (left/right fade)
      const ex = Math.min(1, Math.min(x, BEAM_W - 1 - x) / (BEAM_W * 0.08));
      const idx = (y * BEAM_W + x) * 4;
      d[idx]   = pr;
      d[idx+1] = pg;
      d[idx+2] = pb;
      d[idx+3] = Math.round(ga * ex * 255);
    }
  }
...
export async function exportSaberToSpine(
  ...
  const skinAtts: any   = {};
  const halfH = glowW / 2;
  const ov    = glowW * 0.25;   // small overlap to hide seam at bone joints
