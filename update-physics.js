...
const oldText = c.substring(c.indexOf("if (force.type === 'attractor' |" + "| force.type === 'repulsor') {"), c.indexOf("}\n              });\n            }") + 1);

const newText = \let targetPos = new THREE.Vector3(force.position.x, force.position.y, force.position.z);
...
let ts = c.indexOf("let targetPos = new THREE.Vector3(force.position.x, force.position.y, force.position.z);\n                  if (force.targetShapeId) {");
if (ts !== -1) {
    c = c.substring(0, ts) + c.substring(c.indexOf("}\n", ts) + 2);
}
