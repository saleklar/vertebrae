const fs = require("fs"); let code = fs.readFileSync("src/Scene3D.tsx", "utf8");

const s1 = `let tendrilBaseX = fBase.x, tendrilBaseY = fBase.y, tendrilBaseZ = fBase.z;
            if (pathCurveF) {`;

const r1 = `let tendrilBaseX = fBase.x, tendrilBaseY = fBase.y, tendrilBaseZ = fBase.z;
            let pathFade = 1.0;
            if (pathCurveF) {
...
const s2 = `let activeHeight = flameHeight * ageScale * (0.75 + 0.25 * lifeFade) * sharedHeightScale;
            const baseWidthMul = Math.max(0.05, 1.0 - detachT * 0.9);`;

const r2 = `let activeHeight = flameHeight * ageScale * (0.75 + 0.25 * lifeFade) * sharedHeightScale * pathFade;
