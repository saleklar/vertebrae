...
const wipeStart = code.lastIndexOf('let tendrilBaseX = fBase.x');
const wipeEnd = code.lastIndexOf('// Base spread angle shifts each');

if (wipeStart > -1 && wipeEnd > -1) {
    code = code.substring(0, wipeStart) + \let tendrilBaseX = fBase.x, tendrilBaseY = fBase.y, tendrilBaseZ = fBase.z;
            let pathFade = 1.0;
            if (pathCurveF) {
